...
// field. Mirrors api module's own dev.fleetpulse.api.mqtt.SecuredMosquittoTestSupport;
// duplicated rather than shared because Gradle modules do not share test
// source sets in this repo.
public final class SecuredMosquittoTestSupport {
    ...
    public static GenericContainer<?> newContainer() {
        return new GenericContainer<>(DockerImageName.parse("eclipse-mosquitto:2"))
            ...
            // role/ACL, so this test's own publish raced the real
            // addClientRole call and lost on CI's slower runner (100%
            // reproducible there, essentially never locally). Mosquitto
            // prefixes every *actually executed* dynsec command with
            // "dynsec: " (colon) -- the banner's plain-English usage line
            // never has that colon -- so anchor on that prefix instead.
